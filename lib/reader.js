...
BufferedStreamReader.prototype = {
	...
	read: function(size, cb) {
		...
		if(this.EOF) return cb(null, Buffer(0));
		if(!this.readQueue.length && (this.bufferedLen >= size || this._eof)) {
	...
	_closeStream: function() {
		if(this.stream) {
			...
			if(this.stream.close) this.stream.close(); // TODO: should this be done?
			this.stream = null;
		}
	},
