...
NNTP.prototype = {
	...
	connect: function(cb) {
		...
		this._setTimer(this._onConnectFail.bind(this, new Error('timeout')), this.opts.connTimeout);
	...
	_onConnectFail: function(err) {
		...
		if(this._connectRetries--) {
			...
			this.destroy();
			cb(new Error('NNTP connection failed: ' + err));
		}
	},
	...
	_onError: function(err) {
		...
		if(!this._finished) {
			...
			this._destroy();
			this.connect(this._connectCb); // TODO: handle reconnect error?
		}
	},
	_onData: function(chunk) {
		...
		if(data.length) {
			...
			if(this.dataLength > 4096) {
				this.warn('Received NNTP message larger than 4KB, force disconnecting from server');
				this.socket.destroy();
				this.dataLength = 0;
				this.dataQueue = [];
				return;
	...
	group: function(group, cb) {
		this._doRequest('GROUP ' + group + '\r\n', function(err, code, info) {
			...
			// consider parsing returned packet? not really necessary for our purposes...
