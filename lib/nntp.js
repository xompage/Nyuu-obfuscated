...
var commands = {
	...
	AUTH_USER: new Buffer('AUTHINFO USER ', ENCODING),
	AUTH_PASS: new Buffer('AUTHINFO PASS ', ENCODING),
	DATE: new Buffer('DATE\r\n', ENCODING),
	GROUP: new Buffer('GROUP ', ENCODING),
	CRLF: new Buffer('\r\n', ENCODING)
};
...
NNTP.prototype = {
		...
		async.waterfall([
			...
			function(code, info, cb) {
				...
				if(self.opts.user) {
					...
					self._write(commands.AUTH_USER);
					self._write(self.opts.user);
			...
			function(code, info, cb) {
				if(self.opts.user) {
					...
					if(err) return cb(err);
					self._write(commands.AUTH_PASS);
					self._write(self.opts.password);
					self._request(commands.CRLF, cb);
				} else cb(null, null, null);
			},
			function(code, info, cb) {
			...
			function(cb) {
				...
				if(self.currentGroup) {
					self._write(commands.GROUP);
					self._write(self.currentGroup);
					self._request(commands.CRLF, self._groupResponse.bind(self, cb));
				} else cb();
			}
		], function(err) {
			if(err)
			...
			if(self._requesting) {
				...
				if(self._requestTries > self.opts.requestRetries) {
					...
					(reschedReq.slice(-1)[0])(self._lastDisconnectErr || new NNTPError('request_failed', 'Request retry limit reached'));
				} else {
					// rescheduled request
	...
	_onError: function(err) {
		...
		if(err) {
			...
			this._lastDisconnectErr = err;
		} else {
			if(this._requesting)
				this.warn('NNTP connection unexpectedly lost, reconnecting...');
			else
				this.info('NNTP connection lost');
			this._lastDisconnectErr = new Error('connection lost');
		}
		this._destroy();
		this.numErrors++;
		// TODO: if requesting, consider checking request retry count; currently the check is only performed after successful connect
	...
	post: function(headers, msg, cb) {
		...
		(function doPost() {
			...
			self._doRequest(commands.POST, function(err, code, info) {
				...
				self._setRequesting('post', [headers, msg, cb]);
				self._request([
					headers.join('\r\n') + '\r\nMessage-ID: <' + msgId + '>\r\n\r\n',
					msg
				], function(err, code, info) {
	...
	_setRequesting: function(func, args) {
		if(func) {
