...
NNTP.prototype = {
		...
		if(this.opts.uploadChunkSize && this.opts.uploadChunkSize < msg.length) {
			...
			if(_doWrite()) {
				...
				var onDrain = function() {
					// for updating network post speed
					...
					if(pos >= msg.length) {
						...
						setImmediate(function() { // workaround for bug in node 0.10.x causing excessive process.nextTick calls; bug not present in node 4.7.x
							...
							if(self.state == 'connected' && _doWrite())
								onDrain();
						});
					}
				};
				...
				retLen = 0; // hack to disable network speed update on request completion
			}
		} else {
			if(this._write(msg)) {
				this._clearTimer();
	...
	_requestSetTimer: function(type, time) {
		if(time === undefined) time = this.opts.timeout;
		if(!time) return;
