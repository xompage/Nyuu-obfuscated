...
NNTP.prototype = {
	...
	_onError: function(err) {
		...
		var rf = this._respFunc; // save this for post timeout hack
		this._destroy();
		this.numErrors++;
		
		if(this._requesting) {
			// request retry limit reached?
			if(this._requestTries > this.opts.requestRetries) {
				...
				(reschedReq.slice(-1)[0])(this._lastDisconnectErr || new NNTPError('request_failed', 'Request retry limit reached'), this._requestingEVal);
			} else if(this.opts.ignorePostTimeout && this._requestingFunc == 'post' && err && err.code == 'timeout') {
