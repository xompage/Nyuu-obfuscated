	...
	__post: function(req) {
		...
		(function doPost() {
			...
			req.cbParent = function(err, code, info) {
				if(!err) err = checkExpect(self.opts.useIHave ? 335 : 340, self.opts.useIHave ? 'IHAVE' : 'POST', code, info);
				if(err) return cb(err);
				...
				req.cb = function(err, code, info) {
					...
					var err = checkExpect(self.opts.useIHave ? 235 : 240, 'posted article', code, info);
					if(err) return cb(err, msgId);
