...
function NNTPReq(func, msg, type, cb) {
	this.func = func;
	this.cbParent = cb;
	this.msg = msg;
	this.type = type;
...
NNTP.prototype = {
	...
	_onError: function(err) {
		...
		var postTimeoutHack = (req && this.opts.onPostTimeout && req.post && err && err.code == 'timeout' && req.cb);
		var postTimeoutAction;
		
		if(postTimeoutHack) {
			postTimeoutAction = this.opts.onPostTimeout[req.postTimeouts++];
			if(postTimeoutAction && postTimeoutAction.substr(0, 10) == 'strip-hdr=') {
				var post = this._requests[0].post;
	...
	post: function(post, cb) {
		var req = new NNTPReq('__post', CMD_POST, 'post', cb);
		req.post = post;
		this.__post(req);
	},
	__post: function(req) {
		...
		(function doPost() {
			...
			req.cbParent = function(err, code, info) {
				...
				req.cb = function(err, code, info) {
					...
					if(code == 441) {
						if(req.postRetries++ < self.opts.postRetries) {
							self.warn('Got "' + ('441 ' + info).trim() + '" response when posting article ' + msgId + '; will retry (attempt ' + req.postRetries + '/' + self.opts.postRetries + ')');
							self.numErrors++;
							return doPost();
						}
				...
				req.size = self._uploadPost(post.data);
			};
			self.__doRequest(req);
		})();
	},
	_doRequest: function(msg, type, cb) {
		this.__doRequest(new NNTPReq('__doRequest', msg, type, cb));
	},
	...
	_uploadPost: function(msg) {
		...
		if(this.opts.uploadChunkSize && this.opts.uploadChunkSize < msg.length) {
			// TODO: consider how to convey this to end user's progress bar?
			var pos = 0;
			var onDrain = function() {
				while(pos < msg.length && self._write(msg.slice(pos, pos += self.opts.uploadChunkSize)));
			...
			while(pos < msg.length && this._write(msg.slice(pos, pos += this.opts.uploadChunkSize)));
			if(pos < msg.length)
	...
	_endRequest: function(req) {
		var now = Date.now();
		if(this.state == 'connected') {
			this.reqWaitTime += now - Math.max(this._lastReqTime, req.ts); // note: for cases when server is unresponsive, the slowdown there isn't considered
