...
NNTP.prototype = {
	...
	_onData: function(chunk) {
		...
		this._finished = true;
		if(this.socket) {
			if(this._requesting && this._requestingFunc == 'post' && this.state == 'connected')
				throw new Error('Calling end() whilst posting is not supported');
			this.socket.end('QUIT\r\n');
		}
		this.socket = null;
		if(this.state == 'waiting' || this.state == 'connecting' || this.state == 'authenticating') {
			// re/connecting - cancel it
		...
		else if(this.state != 'inactive' && this.state != 'disconnected')
