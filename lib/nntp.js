...
function NNTP(opts) {
	...
	if(!opts.connect.port) {
		connectOpts = util.extend({}, opts.connect, {port: opts.secure ? 563 : 119});
				...
				*/
				...
				if(!m)
					...
					if(req && req.validResp && req.validResp.indexOf(code) == -1) {
						if(req.type == 'post' && (code == (this.opts.useIHave ? 436 : 441) || (this.opts.useIHave && code == 437))) {
	...
	__post: function(req) {
		...
		(function doPost() {
			...
			req.cbParent = function(err, code, info) {
				...
				req.validResp = (self.opts.useIHave ? [235] : [240]).concat(
					self.opts.postFailReconnect ? [] :
					(self.opts.useIHave ? [436, 437] : [441])
				);
	...
	_uploadPost: function(msg, req) {
		...
		if(this.opts.uploadChunkSize && this.opts.uploadChunkSize < msg.length) {
			...
			if(this._write(msg)) {
				...
				this.socket.once('drain', this._onDrainListener = function() {
					self._onDrainListener = null;
					self._clearTimer();
					self._requestSetTimer('post-data');
				});
