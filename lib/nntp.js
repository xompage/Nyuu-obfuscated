...
NNTP.prototype = {
	...
	_onData: function(chunk) {
		...
		if(data.length) {
			...
			if(this.dataLength > 4096) {
				self[self.state == 'connected' ? '_onError':'_onConnectFail'](new NNTPError('invalid_response', 'Received NNTP message larger than 4KB, force disconnecting from server'));
				return;
			}
				...
				self._request(postData, function(err, code, info) {
					...
					if(code == 441) {
						if(self._postRetries > 0) {
							self.warn('Got "441 ' + info + '" response when posting article ' + msgId + '; will retry');
							self._postRetries--;
							return doPost();
						}
	...
	_request: function(msg, cb) {
		...
		this._setTimer(function() {
			...
			self[self.state == 'connected' ? '_onError':'_onConnectFail'](new NNTPError('timeout', 'Response timed out'));
