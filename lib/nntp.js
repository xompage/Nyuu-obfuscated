...
function NNTPReq(func, args, errVal) {
	this.func = func;
	this.args = args;
	this.errVal = errVal;
}
...
NNTP.prototype = {
		...
		async.waterfall([
			...
			function(cb) {
				...
				if(self.currentGroup && !self._finished) {
					...
					self._respFunc = self._groupResponse.bind(self, cb);
	...
	_onError: function(err) {
		...
		var req = (this._requests.length ? this._requests[0] : null);
		var postTimeoutHack = (req && this.opts.onPostTimeout && req.args.post && err && err.code == 'timeout' && req.cb);
		...
		if(postTimeoutHack) {
			...
			if(postTimeoutAction && postTimeoutAction.substr(0, 10) == 'strip-hdr=') {
				var post = this._requests[0].args.post;
		...
		if(this._requests.length) {
			if((postTimeoutHack || req.tries <= this.opts.requestRetries) && (!postTimeoutHack || postTimeoutAction)) {
				if(err)
					this.warn('NNTP connection error occurred: ' + err.message + '; will retry last request (attempt ' + req.tries + '/' + this.opts.requestRetries + ')');
				else
					this.warn('NNTP connection unexpectedly lost, reconnecting and retrying last request... (attempt ' + req.tries + '/' + this.opts.requestRetries + ')');
			}
		} else {
			if(err)
	...
	// TODO: does this have issues with state, eg, if a request is still processing, will it overwrite the 'closing' state?
		...
		(function(cb) {
			...
			if(self._requests.length && self._requests[0].args.post && self.state == 'connected')
	...
	post: function(post, cb) {
		this.__post(new NNTPReq('__post', {msg: CMD_POST, type: 'post', post: post, postCb: cb}));
	},
	__post: function(req) {
		var post = req.args.post, cb = req.args.postCb;
		...
		(function doPost() {
			...
			req.errVal = null;
			req.args.cb = function(err, code, info) {
				...
				self._setRequesting(req);
				self._request(post.data, 'post-data');
			};
			self.__doRequest(req);
		})();
	},
	_doRequest: function(msg, type, cb) {
		this.__doRequest(new NNTPReq('__doRequest', {msg: msg, type: type, cb: cb}));
	},
	__doRequest: function(req) {
		if(this._isPosting)
			throw new Error('Cannot make request whilst posting');
		else if(req.args.type == 'post')
		...
		this.numRequests++;
		req.tries++;
		this._setRequesting(req);
		if(this.state == 'connected') {
			req.cb = function() {
				this._setRequesting(false);
				req.args.cb.apply(null, arguments);
			}.bind(this);
			this._request(req.args.msg, req.args.type);
		}
	...
	_takeRequest: function() {
		var req = this._requests.shift();
		if(this._requests.length) {
			var nr = this._requests[0];
			this._requestSetTimer(nr.args.type, nr.ts + this.opts.timeout - Date.now());
		}
		return req;
	},
