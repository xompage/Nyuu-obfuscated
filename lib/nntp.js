...
NNTP.prototype = {
		...
		async.waterfall([
			...
			function(cb) {
				...
				if(self.currentGroup && !self._finished) {
					...
					self._request('GROUP ' + self.currentGroup + '\r\n', 'group', self._groupResponse.bind(self, cb));
				} else cb();
			}
		], function(err) {
			if(err)
	...
	_onError: function(err) {
		...
		if(this._requesting) {
			if((postTimeoutHack || this._requestTries <= this.opts.requestRetries) && (!postTimeoutHack || postTimeoutAction)) {
				if(err)
					this.warn('NNTP connection error occurred: ' + err.message + '; will retry last request (attempt ' + this._requestTries + '/' + this.opts.requestRetries + ')');
				else
					this.warn('NNTP connection unexpectedly lost, reconnecting and retrying last request... (attempt ' + this._requestTries + '/' + this.opts.requestRetries + ')');
			}
		} else {
			if(err)
		...
		this._close(function() {
			if(self._requesting) {
				if(postTimeoutHack && postTimeoutAction == 'ignore') {
				...
				} else if((!postTimeoutHack && self._requestTries > self.opts.requestRetries) || (postTimeoutHack && !postTimeoutAction)) {
					...
					(reschedReq.slice(-1)[0])(err || new NNTPError('connection_lost', 'Disconnected during request'), self._requestingEVal);
				}
			}
			
			if(self._requesting || self.opts.keepAlive) {
				setImmediate(self.connect.bind(self, function(err) {
					if(!err) return;
					self._setState('inactive');
					if(self._requesting) {
						// last param is always the callback; not a really nice assumption, but works
						(self._requesting.slice(-1)[0])(err, self._requestingEVal);
					} else {
						self.warn('NNTP connection now inactive due to reconnect failure: ' + err.message);
					}
				}));
			} else {
				self._setState('inactive');
	...
	_onData: function(chunk) {
		...
		if(this._finished) {
			if(cb) cb(); // TODO: need to only call this once actually ended
			return;
		}
		this._finished = true;
		...
		(function(cb) {
			...
			if(self._requesting && self._requestingFunc == 'post' && self.state == 'connected')
				throw new Error('Calling end() whilst posting is not supported');
			self.socket.removeAllListeners('end'); // suppress potential call to _onConnectFail
			self._end(cb, new Buffer('QUIT\r\n', ENCODING));
		})(function() {
			if(cb) cb();
			if(state == 'waiting' || state == 'connecting' || state == 'authenticating') {
				if(self._timer) self._clearTimer();
				self._setState('inactive');
				var rf = (state == 'waiting' ? self._connectCb : self._respFunc);
				if(rf)
					rf(new NNTPError('cancelled', 'Request cancelled'));
			}
		});
	},
	destroy: function() {
		...
		this._finished = true;
		// TODO: is this state=waiting check appropriate?
		var rf = this.state == 'waiting' ? this._connectCb : this._respFunc;
		this._destroy();
		this._setState('inactive');
		if(rf)
			rf(new NNTPError('cancelled', 'Request cancelled'));
	...
	_triggerError: function(code, desc) {
		if(this._finished) {
			...
			if(this._requesting) { // is it even possible for this to be false?
				// last param is always the callback; not a really nice assumption, but works
				(this._requesting.slice(-1)[0])(new NNTPError(code, desc), this._requestingEVal);
			}
		} else {
	...
	date: function(cb) {
		this._doRequest(CMD_DATE, 'date', function(err, code, info) {
	...
	group: function(group, cb) {
		this._doRequest('GROUP ' + group + '\r\n', 'group', this._groupResponse.bind(this, cb));
	},
	...
	post: function(post, cb) {
		...
		(function doPost() {
			...
			self._doRequest(CMD_POST, 'post', function(err, code, info) {
				if(!err) err = checkExpect(340, 'POST', code, info);
				if(err) return cb(err);
				
				// we hard-code a message-id, since it's best to make it unique for retries
				var msgId = post.keepMessageId ?
					post.messageId :
					post.randomizeMessageID();
				
				// mark this request to be retried if disconnected
				self._requestTries = reqRetries;
				self._setRequesting('post', [post, cb], msgId);
				self._request(post.data, 'post-data', function(err, code, info) {
					self._setRequesting(false);
					self._postTimeouts = 0;
					if(err) return cb(err, msgId);
					if(code == 441) {
						if(self._postRetries--) {
							self.warn('Got "' + ('441 ' + info).trim() + '" response when posting article ' + msgId + '; will retry (attempt ' + (self.opts.postRetries-self._postRetries) + '/' + self.opts.postRetries + ')');
							self.numErrors++;
							return doPost();
						}
	...
	_doRequest: function(msg, type, cb) {
		if(this._requesting)
			throw new Error('Request made whilst another request is in progress');
		if(this.state == 'inactive') {
			this.connect(function(err) {
				if(err) cb(err);
				else this._doRequest(msg, type, cb);
			}.bind(this));
			return;
		}
		this._setRequesting('_doRequest', [msg, type, cb]);
		this.numRequests++;
		if(this.state == 'connected') {
			this._request(msg, type, function() {
	...
	_write: function(data) {
		if(typeof data == 'string') {
			this.bytesSent += (this._lastReqSize = Buffer.byteLength(data, ENCODING));
			this.socket.write(data, ENCODING);
		} else {
			this.bytesSent += (this._lastReqSize = data.length);
