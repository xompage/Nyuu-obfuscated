...
NNTP.prototype = {
	...
	_uploadPost: function(msg, req, prefix) {
		...
		if(this.opts.uploadChunkSize && this.opts.uploadChunkSize < msg.length) {
			...
			var _doWrite = function() {
				...
				self._writePostWait(msg.slice(pos, pos += len), function(cancelled) {
					if(cancelled) return;
			...
			this._writePostWait(msg, function(cancelled) {
				if(cancelled) return;
				self._requestSetTimer('post-data');
			});
			return retLen;
	...
	_writePostWait: function(msg, cb) {
		var self = this;
		this.throttle(msg.length, function(cancelled) {
