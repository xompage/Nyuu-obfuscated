...
function Uploader(_opts, cb) {
	...
	this.queue = new Queue(util.optSel(opts.articleQueueBuffer, Math.min(Math.round(this.numPostConns*0.5), 10)), opts.useLazyConnect);
			...
			(function(cb) {
				...
				if(err) {
					...
					if(post.chkFailures >= self.opts.check.tries) {
						...
						// reschedule check
						if(NNTP.log) NNTP.log.debug('Post check failed to find posted article ' + post.messageId + '; will try again later... (attempt ' + post.chkFailures + '/' + self.opts.check.tries + ')');
