...
Uploader.prototype = {
				...
				function doPost(err) {
					...
					if(chkOpts.ulConnReuse && (chkPost = self.checkQueue.takeSync())) {
						...
						var got = self.queue.take(function(post) {
							...
							c.post(post.headerStr, post.data, function(err, messageId) {
								if(err) {
								...
								if(!err) {
									...
									if(self.opts.dumpPosts && messageId) {
										...
										require('fs').writeFile(self.opts.dumpPosts + post.messageId, post.headerStr, function(){});
									}
	...
	cancel: function(cb) {
		// TODO: cancel upload
		// TODO: mark all posted, unchecked articles as complete so that it's written to the NZB
		cb();
