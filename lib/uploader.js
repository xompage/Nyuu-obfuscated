...
Uploader.prototype = {
	...
	_closeConnection: function(type, id) {
		...
		ar[id].end();
		if(type == 'post') {
			this.totalPostSpeed += ar[id].bytesSent / Math.max(ar[id].reqWaitTime, 1);
		}
		ar[id] = null;
	},
	cancel: function(cb) {
	...
	currentPostSpeed: function() {
		...
		this.postConnections.forEach(function(c) {
			// this may over-estimate since bytesSent is incremented immediately, whilst wait time isn't
			if(c) speed += c.bytesSent / Math.max(c.reqWaitTime, 1);
		});
		return speed;
	}
};
