...
Uploader.prototype = {
	...
	start: function() {
		...
		if(this.opts.headerCheck.tries) {
			for(var k in this.opts.server)
				chkSrvOpts[k] = this.opts.server[k];
			for(var k in this.opts.headerCheck.server)
				chkSrvOpts[k] = this.opts.headerCheck.server[k];
			chkGroup = this.opts.headerCheck.group;
			if(!chkGroup) {
			...
			chkReuse = this.opts.headerCheck.ulConnReuse;
		} else
			this.opts.headerCheck.connections = 0;
		async.parallel([
						...
						self.queue.take(function(post) {
							...
							c.post(post.data, function(err, messageId) {
								if(!err) {
									...
									if(self.opts.headerCheck.tries) {
										post.chkFailures = 0;
										self.checkQueue.add(self.opts.headerCheck.checkDelay, post);
									} else {
			...
			async.times.bind(async, this.opts.headerCheck.connections, function(i, cb) {
	...
	_uploadDone: function(err) {
		if(err) {
			...
			if(this.nzb) this.nzbStream.end();
