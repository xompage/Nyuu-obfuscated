...
function Uploader(_opts, cb) {
	...
	this.queue = new Queue(opts.articleQueueBuffer || this.numPostConns, opts.useLazyConnect);
	this.checkQueue = new TimerQueue(opts.check.queueBuffer || Math.min(this.numPostConns*8, 100), opts.useLazyConnect);
...
Uploader.prototype = {
	...
	_start: function() {
		...
		async.each(this.opts.servers, function(server, cb) {
			async.parallel([
