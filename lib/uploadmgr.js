...
function UploadManager(_opts) {
	...
	this.uploader = new Uploader(opts, this.onPosted.bind(this));
}
UploadManager.prototype = {
	...
	addFile: function(fileName, fileSize, fileNum, fileNumTotal, postHeaders, stream, fileDone) {
		...
		(function readLoop() {
			stream.read(self.opts.articleSize, function(err, buffer) {
				...
				if(self.nzb) {
					...
					if(!nzbFile) {
						nzbFile = self.nzb.file(
							...
							Math.ceil(fileSize / self.opts.articleSize)
						);
					}
				...
				self.uploader.addPost(post, readLoop);
			});
		})();
	},
	...
	finished: function(cb) {
		...
		this.uploader.finished(function(err) {
			if(this.nzb) {
				if(!err) this.nzb.end();
				try {
					this.nzbStream.end();
