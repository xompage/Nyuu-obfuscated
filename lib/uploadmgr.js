...
function UploadManager(opts, cb) {
	...
	if(opts.useBufferPool !== false) {
		...
		this.bufferPool = new BufferPool(ArticleEncoder.maxSize(this.articleSize, opts.bytesPerLine) + opts.headerAllocSize, BufferPool.calcSizeForUpload(this.uploader, opts.server.connections));
	}
