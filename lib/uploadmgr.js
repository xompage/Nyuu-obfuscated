...
function UploadManager(opts, cb) {
	...
	if(opts.useBufferPool !== false) {
		...
		this.bufferPool = new (BufferPool || (BufferPool = require('./bufferpool')))(ArticleEncoder.maxSize(this.articleSize, opts.bytesPerLine) + opts.headerAllocSize, maxPoolSize);
	}
...
UploadManager.prototype = {
	...
	addFile: function(fileName, fileSize, fileNum, fileNumTotal, postHeaders, stream, fileDone) {
		var enc = new ArticleEncoder(fileName, fileSize, Math.ceil(fileSize / this.articleSize), this.opts.bytesPerLine, this.dateOverride);
		var sizes = [];
		var self = this;
		var nzbFile;
