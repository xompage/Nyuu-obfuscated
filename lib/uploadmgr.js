...
var reloadPost = function(post, stream, size, cb) {
	if(post.data) throw new Error('Attempt to reload post that already has been loaded');
	stream.readRange((post.part-1) * size, new Buffer(size) /* TODO: use Buffer pool */, function(err, data) {
		...
		(function readLoop() {
			stream.read(self.articleSize, function(err, buffer) {
				...
				if(stream.readRange) // reloadable post
					post.reload = reloadPost.bind(null, post, stream, self.articleSize);
