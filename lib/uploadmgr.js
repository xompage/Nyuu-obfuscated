...
var subject_func = function(pre, post, filenum, filenumtotal, filename, filesize, part, parts, size) {
...
UploadManager.prototype = {
	...
	addFile: function(fileName, fileSize, fileNum, fileNumTotal, postHeaders, stream, fileDone) {
		...
		if(headers[subject_key] === null) {
			...
			postSubj = '/' + enc.parts + ') ' + fileSize + (this.opts.comment2 ? ' ' + this.opts.comment2 : '');
			
			headers[subject_key] = subject_func.bind(null, preSubj, postSubj);
		}
		...
		(function readLoop() {
			stream.read(self.articleSize, function(err, buffer) {
				...
				var post = enc.generate(buffer, self.bufferPool);
