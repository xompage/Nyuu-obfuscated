...
MultiEncoder.prototype = {
	...
	setHeaders: function(headers) {
		...
		for(var h in headers) {
			...
			else if(h.toLowerCase() == 'date' && !v && this.timestamp)
	...
	generate: function(data, pool, grabHeaders) {
		...
		for(var h in this.headers) {
			...
			_headers[h] = v;
			if(grabHeaders) grabHeaders[h.toLowerCase()] = v;
		}
...
Post.prototype = {
	...
	getHeader: function(str) {
		var headers = this.data.slice(0, this.postPos).toString(ENCODING);
	...
	stripHeader: function(str) {
		...
		var headers = this.data.slice(0, this.postPos).toString(ENCODING).toLowerCase();
	...
	while(1) {
		...
		for(var h in headers) {
			...
			if(h.toLowerCase() == 'date' && !v)
				v = (new Date(this.genTime)).toUTCString();
			
			this.postPos += this.buf.write(h + ': ' + v + '\r\n', this.postPos, ENCODING);
		}
