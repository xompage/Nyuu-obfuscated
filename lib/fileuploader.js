	...
	async.eachSeries(_files, function(file, cb) {
		...
		async.eachSeries(filenames, function(filename, cb) {
			...
			if(file.archive) {
				...
				if(!stream)
					stream = fs.createReadStream(filename, {autoClose: false, highWaterMark: opts.diskReqSize});
				else if(typeof stream == 'function') { // to support deferred loading
					stream = stream();
				}
				if((typeof stream != 'object') || !stream.readable)
					return cb(new Error('Cannot read from file ' + file.name));
				
				var r = new Reader(stream, opts.diskBufferSize);
