	...
	async.eachSeries(_files, function(file, cb) {
		...
		async.eachSeries(filenames, function(filename, cb) {
			...
			if(file.archive) {
				...
				var reader;
				if(!file.stream) {
					reader = new FileReader(filename, opts.diskReqSize || opts.articleSize, opts.diskBufferSize);
				} else {
					...
					reader = new Reader(stream, opts.diskBufferSize);
				}
				ee.emit('processing_file', file);
				
				up.addFile(file.name, file.size, file.num, fileColCount[file.collection], reader, function(err, info) {
					reader.close();
					cb(err);
				});
			}
