	...
	async.eachSeries(_files, function(file, cb) {
		...
		async.eachSeries(filenames, function(filename, cb) {
			...
			if(file.archive) {
				...
				if(!stream)
					stream = fs.createReadStream(filename, {autoClose: false, highWaterMark: opts.diskReqSize || opts.articleSize});
				...
				up.addFile(file.name, file.size, file.num, fileColCount[file.collection], r, function(err, info) {
					r.close();
					// fs.ReadStream doesn't have an explicit close mechanism, annoyingly... http://stackoverflow.com/questions/19277094/how-to-close-a-readable-stream-before-end
					if(stream.close)
						stream.close(function(err2) {
							cb(err || err2);
						});
					else
						cb(err);
